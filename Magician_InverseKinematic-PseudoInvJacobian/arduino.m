classdef arduino < handle
    ...
    methods
        ...
        % digital write
        function robot_magican(a,theta1,theta2,theta3,tg)
            ...
            if a.chkp,
                ...
                % check nargin
                if nargin~=5,
                    error('Function must have the "theta1,theta2,theta3" and "time" arguments');
                end
                
                % check theta1
                errstr=arduino.checknum(theta1,'theta1',0:180);
                if ~isempty(errstr), error(errstr); end
                
                % check theta2
                errstr=arduino.checknum(theta2,'theta2',15:118);
                if ~isempty(errstr), error(errstr); end
                
                % check theta1
                errstr=arduino.checknum(theta3,'theta3',60:157);
                if ~isempty(errstr), error(errstr); end
                
                % check tg
                errstr=arduino.checknum(tg,'time',1:99);
                if ~isempty(errstr), error(errstr); end
            ...
            %%%%%%%%%%%%%%%%%%%%%%%%% PERFORM DIGITAL OUTPUT %%%%%%%%%%%%%%%%%%%%%%%%%%
                ...
                % send mode and byte
                fwrite(a.aser,[86 48+theta1 48+theta2 48+theta3 48+tg],'uchar');
